...
### .... buy 0.001  bitcoin
# Example: ./krakenapi.py AddOrder pair=xxbtzusd type=buy ordertype=market volume=0.001 leverage=5
### ... sell bitcoin for 15$ (oflags because oflag is set )
# ! viqc flag currently unavilable
# Example: ./krakenapi.py AddOrder pair=xxbtzusd type=buy ordertype=market oflags=viqc volume=15.72
### to have a workaround we need to get the current value
### eg 50euro in xrp
...
if api_method in api_private or api_method in api_trading or api_method in api_funding:
    ...
    try:
        ...
        print("Public Key: {}".format(api_key))
        api_secret = base64.b64decode(open("API_Private_Key").read().strip())
        print("Private Key: {}".format(api_secret))
    except:
